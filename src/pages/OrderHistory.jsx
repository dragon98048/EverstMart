...
function OrderHistory() {
  ...
  const loadOrders = async () => {
    try {
      ...
      const { data } = await axios.get('http://localhost:5000/api/order-history', {
  ...
  const reorder = async (orderId) => {
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:5000/api/order-history/${orderId}/reorder`,
        {},
        { headers: { Authorization: `Bearer ${token}` }}
      );
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/order-history/${orderId}/cancel`,
        { reason },
        { headers: { Authorization: `Bearer ${token}` }}
      );
